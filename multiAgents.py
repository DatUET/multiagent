...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        successorGameState = currentGameState.generatePacmanSuccessor(action) #tạng thái tiếp theo của Pacman
        newPos = successorGameState.getPacmanPosition() #lấy vị trí tiếp theo của Pacman
        newFood = successorGameState.getFood() #lấy danh sách các thức ăn còn trên map
        newGhostStates = successorGameState.getGhostStates() # trả về data của ghost
        newScaredTimes = [ghostState.scaredTimer for ghostState in newGhostStates]

        "*** YOUR CODE HERE ***"

        value = successorGameState.getScore() #điểm tại vị trí tiếp theo

        mul2DistanceToGhost = manhattanDistance(newPos, newGhostStates[0].getPosition()) # khoảng cách mahattan từ pacman đến ghost đầu
        # print "mahattan 0" , manhattanDistance(newPos, newGhostStates[0].getPosition())
        # print "mahattan 1" ,manhattanDistance(newPos, newGhostStates[1].getPosition())
        if mul2DistanceToGhost > 0:
            value -= 10.0 / mul2DistanceToGhost
...
def betterEvaluationFunction(currentGameState):
    ...
    "*** YOUR CODE HERE ***"
    """
        Nếu trạng thái game là win or lose -> trả về inf
    ...
    if len(currentGameState.getCapsules()) != 0:
        closestCapsule = min([manhattanDistance(capsule, newPos) for capsule in currentGameState.getCapsules()]) # khoảng cách ngắn nhất đến thức ăn to
        capsuleScore += 200 / closestCapsule

    closestghost = min([manhattanDistance(newPos, ghost.getPosition()) for ghost in newGhostStates]) # khoảng cách ngắn nhất đến ma
